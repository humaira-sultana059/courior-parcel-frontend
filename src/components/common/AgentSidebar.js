...
export default function AgentSidebar() {
  ...
  const menuItems = [
    ...
    { label: "QR Scanner", href: "/agent/scan", icon: "📱" }, // added QR Scanner link
    { label: "Delivery Route", href: "/agent/route", icon: "🗺️" },
  ];
  ...
  return (
    <>
      ...
      `}
      >
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 to-purple-800 opacity-50"></div>
            ...
            >
              <div
                className={`flex items-center gap-3.5 px-4 py-3.5 rounded-xl transition-all duration-300 group ${
                  isActive(item.href)
                    ? "bg-[#1e293b] text-purple-400 shadow-inner shadow-black/20 border-l-4 border-purple-500"
